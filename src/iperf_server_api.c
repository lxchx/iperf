...
cleanup_server(struct iperf_test *test)
{
    ...
    SLIST_FOREACH(sp, &test->streams, streams) {
        ...
        if (sp->thread_created == 1) {
            ...
            /* Android emulates pthread_cancel() with pthread_kill(). After
             * sp->done = 1, the worker can notice the flag and exit on its
             * own before this call, and Android may then report the stale
             * pthread_t as EINVAL rather than ESRCH. There is no reliable
             * portable way to probe pthread_t liveness here, so treat Android
             * EINVAL as benign. */
            if (rc == EINVAL) {
                rc = ESRCH;
            }
